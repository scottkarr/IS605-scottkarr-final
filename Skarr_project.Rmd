---
title: 'DATA 605 Final Project - Prediction Model'
author: "Scott Karr"
date: "Dec 23, 2016"
output: 
  html_document: 
...
AmesHomes$MSZoning[is.na(AmesHomes$MSZoning)] <- 'C (all)'

# impute 2 nulls for Utilities with 'AllPub'
AmesHomes$Utilities[is.na(AmesHomes$Utilities)] <- 'AllPub'
...
AmesHomes$Exterior1st[is.na(AmesHomes$Exterior1st)] <- 'BrkFace'

# impute 1 nulls for Exterior2nd with 'None'
AmesHomes$Exterior2nd[is.na(AmesHomes$Exterior2nd)] <- 'BrkFace'

# impute 1 nulls for Functional with 'None'
AmesHomes$Functional[is.na(AmesHomes$Functional)] <- 'Typ'
...
#### Factor Imputations
Electrical(43) 1, MasVnrType(26) 8, BsmtQual(31) 57, BsmtCond(32) 57, BsmtFinType1(34) 57, BsmtExposure(33) 58, BsmtFinType2(36) 58, GarageType(59) 81, GarageFinish(61) 81, GarageQual(64) 81, GarageCond(65) 81, FireplaceQu(58) 690
Fence(74) 1179, Alley(7) 1369, MiscFeature(75) 1406, PoolQC(73) 1453

\newpage

#Descriptive Statistics 

Significant skew and kurtosis exists for the attributes LotArea, LowQualFinSF, X3SsnPorch, ScreenPorch, PoolArea and MiscVal.
These attributes are indicative of an uncommon occurance in the data set.  The model will be determinative of whether these attributes truly represent "outliers" or are uncommon but valuable to include in the model.
...
Catagorical data is converted to factor data for inclusion in the model and represented below as proportional
bar plots to indicate their relative relation to housing price.
...
Ordinal attributes are show as histograms.  Most of these attributes contain enough cases that even when significant skew occurs
they are still useful to an ordinary least squares model.  The significance of each attribute will be tested by p-value, AIC and vif using the lm model in step 3.
...
# Stepwise Regression
lm.full <- formula(lm(SalePrice~.,AmesHomes))
lm.null <- lm(SalePrice ~ 1, data = AmesHomes)
model.aic.both <- step(lm.null, direction = "both", trace = 1, scope = lm.full)
```
...
# impute 5 nulls for MSZoning with 'None'
AmesHomes.t$MSZoning[is.na(AmesHomes.t$MSZoning)] <- 'C (all)'

# impute 2 nulls for Utilities with 'None'
AmesHomes.t$Utilities[is.na(AmesHomes.t$Utilities)] <- 'AllPub'

# impute 2 nulls for Functional with 'None'
AmesHomes.t$Functional[is.na(AmesHomes.t$Functional)] <- 'Typ'
...
# impute 1 nulls for Exterior1st with 'None'
AmesHomes.t$Exterior1st[is.na(AmesHomes.t$Exterior1st)] <- 'BrkFace'

# impute 1 nulls for Exterior2nd with 'None'
AmesHomes.t$Exterior2nd[is.na(AmesHomes.t$Exterior2nd)] <- 'BrkFace'
...
# impute 1 nulls for TotalBsmtSF with 'None'
AmesHomes.t$KitchenQual[is.na(AmesHomes.t$KitchenQual)] <- 'Gd'

# impute 1 null for KitchQual with 'Gd'
# Show Descriptive Statistics
...
```{r, eval=TRUE, include=FALSE}
f <- predict(model.aic.both, newdata=AmesHomes.t)
prediction_df <- data.frame(AmesHomes.t$Id, f)
colnames(prediction_df) <- c('Id','SalePrice')
